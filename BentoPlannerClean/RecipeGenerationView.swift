    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            if bentoStore.isLoading {
                loadingView
            } else if bentoStore.aiGeneratedRecipes.isEmpty {
                emptyStateView
            } else {
                recipesListView
            }
        ...
        .onAppear {
            // 初回表示時に自動生成
            if bentoStore.aiGeneratedRecipes.isEmpty {
                Task {
    ...
    var recipesListView: some View {
        ScrollView {
            LazyVStack(spacing: 16) {
                ForEach(bentoStore.aiGeneratedRecipes) { recipe in
        ...
        Button(action: {
            ...
            .background(
                RoundedRectangle(cornerRadius: 12)
                    .fill(bentoStore.isLoading ? Color.gray : Color.orange)
            )
